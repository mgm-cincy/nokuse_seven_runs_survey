...
2025-10-21 double checking the length of stream meters in sample frame, sr_sf, versus the survey weights of the sites

2025-10-20 checking structure of sites_stratv3 as left merge in script 02 not working

2025-10-03 trying to adjust weights given new vignettes in spsurvery v 5.6.0

2025-09-17 survey was implemented from September 9-12th, and not all sites met target population criteria so weights will need to be re-adjusted
...
## Flowlines and Upstream Tributaries Trace
...
srsf_rds <- saveRDS(sr_sf, file = "outputs_from_code/srsf.rds")
...
mapview(sr_sf) + mapview(basin)

saveRDS(basin, file = "outputs_from_code/basin.rds")
...
Draw different types of survey designs and evaluate which we want to use. The stratified equal probability survey design was chosen.

Marc Weber provided the two ways to summarize the sf object srs_sf for length, which requires setting the units.
...
# switch to meters for sample frame of stream to compare to meters of probabilisticly drawn sites
sr_sf$length_m <- units::set_units(st_length(sr_sf),m)

sr_sf %>% 
  group_by(stream_type) %>% 
  summarize(sum(length_m))

sr_sf %>%
  group_by(stream_type) %>%
  summarize(total_length = sum(units::set_units(st_length(geometry),m)))
...
plot(strata_eqprob, sr_sf, key.width = lcm(3))
...
mapview(strata_eqprob$sites_base) + mapview(strata_eqprob$sites_over, color = "red") + mapview(sr_sf, zcol = "stream_type") + mapview(nkbnd)


# 3.3 proportional inclusion probabilities based on lengthkm
...
## Combine Base & Overdraw Sites

From <https://usepa.github.io/spsurvey/articles/weighting.html> use sp_rbind to bind rows of base and overdraw samples as all sites need to be evaluated in a survey.

Note that zooming into Mapview with OpenTopoMap as base map, we should have suspected that downstream n_untrib_3 not in target population as symbology suggests it might be a pond. A download of NDHPlus v2 waterbodies might confirm if USGS had also indicated that site was a pond.

```{r base_overdraw}

sites_strat <- sp_rbind(strata_eqprob)

names(sites_strat)

sites_stratv2 <- dplyr::select(sites_strat, c(siteID:lengthkm, stream:geometry))
...
Remove columns and save sample frame of base and overdraw sites. Write as .csv file so I can add entries for evaluation categories and site names. Also, save as .rds file so I can bring it back in as an sf object.

Check that base site weights match the length of stream meters for the 4 levels of stratification.

```{r remove_columns_site_weights}

sites_stratv3 <- dplyr::select(sites_strat, c(siteID:ip, abbreviation:comment)) %>% print(n=Inf)

write.csv(sites_stratv3, file = "outputs_from_code/sites_stratv3.csv")

saveRDS(sites_stratv3, file = "outputs_from_code/sites_stratv3.rds")

# can just load the saved seven runs base and overdraw sites
sites_stratv3 <- readRDS("outputs_from_code/sites_stratv3.rds")

dplyr::filter(sites_stratv3, siteuse == "Base") %>% 
  group_by(., stratum) %>% 
  summarize(sum(wgt))

mapview(sites_stratv3)

mapview(sites_stratv3) %>%
  addStaticLabels(label = sites_stratv3$abbreviation,
                  noHide = TRUE,
                  direction = 'top',
                  textOnly = TRUE,
                  textsize = "20px")
